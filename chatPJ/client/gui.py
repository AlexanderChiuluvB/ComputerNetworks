...
class LoginApp(Tkinter.Frame):
    ...
    def login(self):     # 登录事件
        if self.id_input.get() == '':
            ...
            return
        try:
            self.client.sock.connect() # 连接服务器
            self.client.sock.sendData('0','3',self.id_input.get() + '@@' + self.passwd_input.get())   # 发送登陆信息
        except IOError:
...
class Register_application(Tkinter.Frame):       #注册界面
    ...
    def summit(self):
        ...
        if self.passwd_input.get().find('@') != -1:
            ...
            return
        try:
            self.client.sock.connect()
            self.client.sock.sendData('0','1',self.id_input.get() + '@@'+ self.passwd_input.get())
        except IOError:
            tkMessageBox.showerror('错误','无法连接服务器')
            self.quit()
            return
...
class Main_Room_application(Tkinter.Frame):
    ...
    def createWidgets(self):
        self.master.geometry('1200x800+300+75')
        ########################################################
        ...
        self.allroom = Tkinter.Listbox(self,yscrollcommand = self.scrollbar.set,height = 3)
        ...
        self.messscrollbar.grid(row = 2,column =4,sticky = Tkinter.W,ipady = 50)
        ...
        self.inputlabel = Tkinter.Label(self,text = '发送信息:')
        ...
        self.input_str.grid(row=5, column=3, ipadx = 200, ipady = 50,sticky=Tkinter.N)
        ####################################################
    ...
    def entry_room(self):
        ...
        if tmp_room in self.client.room_app:
            return
        self.client.sock.sendData('0', '4', str(self.allroom.get(self.allroom.curselection()[0])) )  # 发送进入房间消息
        top = Tkinter.Toplevel(self)
        new_room = Main_Chat_application(master = top,clientp = self.client,room_name = tmp_room,user_name = self.username)
    ...
    def send_mess(self):
        if self.input_str.get() == '':
            return
        self.client.sock.sendData('1','1',self.input_str.get())
    ...
    def create_person_chat(self,event):
        ...
        if tmp_name in self.client.person_app:
            return
    ...
    def create_new_person(self):  # 消息来时弹出窗口
        ...
        if per_name in self.client.person_app:
            return
        top = Tkinter.Toplevel(self)
        new_person = Main_Person_application(master=top, clientp=self.client,
    ...
    def back_login(self):
        ...
        self.client.sock.sendData('1','5',self.username)
        self.master.destroy()
...
class  Main_Chat_application(Tkinter.Frame):
    ...
    def createWidgets(self):
        ...
        self.input_text.grid(row = 2, column = 1,ipadx = 300,ipady = 10,sticky =Tkinter.W)

        self.all_room_user = Tkinter.Listbox(self)
        #self.all_room_user.bind('<Double-Button-1>',self.kick)         # 双击事件
        self.all_room_user.grid(row=0, column=2, sticky=Tkinter.N, ipady=100)

        self.client.room_user = self.all_room_user

        self.sendKickButton = Tkinter.Button(self,text= '踢走用户',command = self.kick)
        self.sendKickButton.grid(row  = 1 ,column = 2,sticky = Tkinter.W,ipadx = 10 ,ipady = 10)

        self.sendButton = Tkinter.Button(self,text= '发送消息',command = self.send_mess)
        self.sendButton.grid(row = 2 ,column = 2,sticky = Tkinter.W,ipadx = 10 ,ipady = 10)

        self.backButton = Tkinter.Button(self,text = '退出房间',command = self.back_room)
        self.backButton.grid(row = 3, column = 2,sticky = Tkinter.W,ipadx=10,ipady=10)
    ...
    def kick(self):
        ...
        if self.username == 'admin':
            if str(self.all_room_user.curselection()) == '()':
                tkMessageBox.showinfo('提示','未选择房间')
                return

            kick_user = str(self.all_room_user.get(self.all_room_user.curselection()[0]))

            print("kick user",kick_user)

            self.client.sock.sendData('1','4',self.room_name+'#'+kick_user)

            return
        else:
            tkMessageBox.showerror("提示","您不是管理员,无资格把别人提出去")
            return


    def back_room(self):
        print ('用户退出房间:' + self.room_name)
        self.client.sock.sendData('0','5',self.room_name)  # 发送退出房间消息
        if self.room_name in self.client.room_app :
    ...
    def send_mess(self):
        if self.input_text.get() == '':
            return
        self.client.sock.sendData('1','2',self.room_name + '@@' + self.input_text.get())
...
class New_room_application(Tkinter.Frame):
    ...
    def new_room(self):
        ...
        self.client.sock.sendData('0','6',self.id_input.get())
...
class Main_Person_application(Tkinter.Frame):
    ...
    def back_chat(self):
        ...
        if self.person_name in self.client.person_app:
    ...
    def send_mess(self):
        if self.input_text.get() == '':
            return
        self.client.sock.sendData('1','3',self.person_name + '@@' + self.input_text.get())
        print (self.input_text.get())
